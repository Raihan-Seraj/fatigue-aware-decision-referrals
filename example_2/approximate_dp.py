...
'''
def run_dp_parallel_beta(num_tasks_per_batch, mu, lamda, w_0, sigma_a, H0, H1, prior, d_0, alpha, beta, sigma_h, ctp, ctn, cfp, cfn, cm, num_bins_fatigue, T, num_expectation_samples,result_path):
    
    ut = Utils(num_tasks_per_batch, mu, lamda, w_0, sigma_a, H0, H1, prior, d_0, alpha, beta, sigma_h, ctp, ctn, cfp, cfn, cm, num_bins_fatigue)

    run_info = wandb.init(project="Example 2",name="beta "+str(beta)+' alpha '+str(alpha)+' mu '+str(mu)+' lambda '+str(lamda),settings=wandb.Settings(start_method="fork"))
    ...
    path_name = result_path + 'num_tasks '+str(num_tasks_per_batch)+'/beta '+str(beta)+'/alpha '+str(alpha)+'/mu_'+str(mu)+'_lambda_'+str(lamda)+'/'
...
def main():
    ...
    parser.add_argument('--beta', type=float, default= 0.5, help='The effect of fatigue on the human observation channel')
    ...
    run_dp_parallel_beta(num_tasks_per_batch, mu, lamda, w_0, sigma_a, H0, H1, prior, d_0, alpha, beta, sigma_h, ctp, ctn, cfp, cfn, cm, num_bins_fatigue, T, num_expectation_samples,result_path)
    ...
    EVAL.run_perf_eval(alpha, beta, result_path, lamda_new, T, num_tasks_per_batch)
    ...
    EVAL.compute_performance(alpha,beta, result_path, lamda_new, T, num_tasks_per_batch)
